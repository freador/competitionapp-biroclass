...
**Stack:** Rails 8 backend e views com Tailwind; PostgreSQL; Deploy Railway (web + Solid Queue worker + volume `/storage`); autenticação nativa com `has_secure_password`; convites via links únicos (sem SMTP); Solid Queue/Solid Cable integrados.
...
**Fluxos & Regras:** criador configura competição, adiciona jurados/participantes e publica; admin gerencia ciclo de vida; jurado/participante aceita convite através de link (`/invites/:token`) e cria conta se necessário; visitantes só acessam dashboards públicos. Regras-chave: publicar exige ≥1 jurado e participante; cada jurado avalia um participante uma vez; rankings atualizam com jobs Solid Queue quando `Note` é criada/atualizada; ao encerrar, notas travam edição e convites podem ser revogados removendo o token.
...
**Roadmap MVP (2 semanas):** Semana 1 cobre setup Rails/Tailwind, modelos, auth, CRUD e convites via link; Semana 2 entrega notas, ranking automático (`total_score = notes.average(:score)`), página pública, deploy Railway, testes e ajustes. Extensões futuras: uploads multimídia, votação pública, mobile app, comentários, ranking ponderado e fases múltiplas.

## Convites via Link
Owners gerenciam convites no dashboard da competição: cada jurado/participante possui `invitation_token` com link pronto (`invite_path(token: ...)`). Compartilhe manualmente (email, chat etc.). O convidado acessa `/invites/:token`, cria conta (ou faz login) e automaticamente ganha o papel certo. Para revogar acesso, remova o participante ou `CompetitionJudge` associado—o token deixa de funcionar. Ajuste o wizard sempre que mudar o formato dos campos (ex.: participantes podem ser digitados como `Nome <email>` para pré-preencher).
